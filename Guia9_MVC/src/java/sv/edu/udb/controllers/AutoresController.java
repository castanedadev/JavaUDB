...
import sv.edu.udb.model.AutoresModel;
import sv.edu.udb.utils.Validaciones;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (operacion) {
            case "listar":
                ...
                break;
            case "nuevo":
                System.out.println("forward to nuevoAutor");
    ...
    private void obtener(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (miAutor != null) {
                ...
                response.sendRedirect(request.getContextPath() + "/WEB-INF/error404.jsp");
            }
            ...
            if (listaErrores.size() > 0) {
                ...
                if (modelo.modificarAutor(miAutor) > 0) {
                    ...
                    request.getSession().setAttribute("fracaso", "El autor no ha sido modificado"
                            + "ya hay un autor con este codigo ");
                    response.sendRedirect(request.getContextPath() + "/autores.do?op=listar");
                }
            }
        } catch (IOException | SQLException | ServletException ex) {
